<script>import { PACKAGE_NAME_AND_VERSION, documentPointerUp$, packageMetadata } from '@w2k/core';
import { filter, map } from 'rxjs';
import { onDestroy } from 'svelte';
import { Observer } from 'svelte-rxjs-observer';
import { w2kDisplaySettingsIconLarge, w2kProgramsIconLarge, w2kScanIconLarge, w2kShutdownIconLarge, } from '../assets/icons';
...
const closeEffect = desktopSlice.desktop$.createEffect(documentPointerUp$.pipe(filter((event) => {
    if (event.button !== 0) {
        return false;
    }
    const elementsUnderPointer = document.elementsFromPoint(event.pageX, event.pageY);
    return ((!elementsUnderPointer.includes(startMenu) &&
