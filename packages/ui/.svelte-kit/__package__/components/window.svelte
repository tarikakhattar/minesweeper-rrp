<script>import { Subject, Subscription, concat, filter, interval, map, of, startWith, switchMap, take, tap, } from 'rxjs';
...
import { formatAnimationVariables } from './taskbar-animation';
import TitleBar from './title-bar.svelte';
import { initialWindowState } from './window-state.interface';
export let windowElement = undefined;
export let grippy = undefined;
const dispatch = createEventDispatcher();
export let windowState = undefined;
...
export const errorNotification = new Subject();
export const errorFlash$ = errorNotification.pipe(switchMap(() => concat(of(false), interval(66).pipe(take(6), map((_, i) => i % 2 === 0)), of(undefined))), tap(() => {
    if (transient) {
        activate();
    }
}), startWith(undefined));
...
onMount(async () => {
    await sleep(0);
    const windowPlane = document.querySelector('#window-plane');
...
const getMaximizeAnimation = (windowState, stage) => {
...
<style>.program-window {
  position: relative;
  box-sizing: border-box;
  user-select: none;
  touch-action: none;
}
.program-window.minimizing, .program-window.unminimizing, .program-window.minimized {
  visibility: hidden !important;
}
.program-window .window-body {
  overflow: auto;
  margin: 0;
}
.program-window.fit-content {
  display: table;
}
.program-window:not(.fit-content) {
  display: flex;
  flex-direction: column;
}
.program-window:not(.fit-content) .window-body {
  height: 100%;
}
.program-window .menu {
  min-height: 20px;
...
.program-window .menu :global(button::first-letter) {
  text-transform: uppercase;
}
.program-window.maximized, .program-window.restoring {
...
.program-window:not(.maximized) {
  height: fit-content;
  width: fit-content;
  min-width: fit-content;
  min-height: fit-content;
}
.program-window.invisible {
  opacity: 0;
}</style>
