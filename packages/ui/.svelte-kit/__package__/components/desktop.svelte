<script>import { packageMetadata } from '@w2k/core';
import { onDestroy, onMount } from 'svelte';
import { desktopColorSchemeToCssVariables, joinStyleMap } from '..';
import { readGlobal, writeGlobal } from '../helpers';
    ...
    selectionDraggable = grippy.draggable({
        ...
        listeners: {
            ...
            move: (e) => {
                ...
                const maxWidthBoundary = cursorX > e.cursor.origin.x
                    ? workspaceRect.x + workspaceRect.width
                    : workspaceRect.x;
                const maxHeightBoundary = cursorY > e.cursor.origin.y
                    ? workspaceRect.y + workspaceRect.height
                    : workspaceRect.y;
                const maxWidth = Math.abs(e.cursor.origin.x - maxWidthBoundary + workspaceRect.x);
                const maxHeight = Math.abs(e.cursor.origin.y - maxHeightBoundary + workspaceRect.y);
