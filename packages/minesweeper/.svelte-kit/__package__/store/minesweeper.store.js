...
import { Slice, entitySliceReducer, entitySliceReducerWithPrecompute, ifLatestFrom, } from '@tinyslice/core';
import { Coordinate } from '@w2k/common';
import { Observable, combineLatest, distinctUntilChanged, filter, map, merge, of, shareReplay, startWith, switchMap, take, takeUntil, timer, withLatestFrom, } from 'rxjs';
import { documentMouseLeave$, documentPointerDown$, documentPointerUp$ } from '@w2k/core';
import { GameState, isGameLost, isGameOngoing, isGameReadyToStart, isGameWon, } from '../interfaces/game-state.enum.js';
import { TileMark, getNextTileMark, isEmptyTileMark, isFlagTileMark, isQuestionTileMark, isTheSamePreset, } from '../interfaces/index.js';
import { SmileyState, } from './minesweeper.interface.js';
export const MS_TAG = '[minesweeper]';
/**
 * Take all tiles, shuffle them, take the first n amount, those will be the mines
 */
const selectNRandomTiles = (tiles, safeCoordinate, amount) => {
    const tilesCopy = tiles.filter((tile) => tile.x !== safeCoordinate.x || tile.y !== safeCoordinate.y);
    shuffleArray(tilesCopy);
    return tilesCopy.splice(0, amount).map((tile) => ({ x: tile.x, y: tile.y }));
};
const isAWinState = (tiles) => tiles.every((tile) => (tile.isMine && !tile.revealed) || (!tile.isMine && tile.revealed));
const isALoseState = (tiles) => tiles.some((tile) => tile.isMine && tile.revealed);
const revealEndStateReducer = (tiles, revealedTileKey) => Object.entries(tiles).reduce((acc, [tileKey, tile]) => {
...
const generateGameInstance = (settings) => {
    const gameInstanceInitialState = {
        settings,
        elapsedTime: 0,
        clickCount: 0,
...
const isBaseWindowState = (o) => {
    return Object.hasOwn(o, 'processId') && Object.hasOwn(o, 'title') && Object.hasOwn(o, 'active');
};
const isWindowSlice = (slice) => {
    return isBaseWindowState(slice.value);
};
export const createMineSweeperGame = (parentSlice, key) => {
    let windowSlice;
    ...
    const minesweeperActions = {
        ...
        resetGame: game$.createAction(`${MS_TAG} reset`),
        startGame: game$.createAction(`${MS_TAG} start game`),
        setPreset: game$.createAction(`${MS_TAG} set preset`),
        addGameToHistory: game$.createAction(`${MS_TAG} add game to history`),
        incrementTimer: game$.createAction(`${MS_TAG} increment timer ms`),
        tileActions: {
            revealTile: game$.createAction(`${MS_TAG} ${TILE_TAG} reveal`),
            depressTile: game$.createAction(`${MS_TAG} ${TILE_TAG} depress`),
            markTile: game$.createAction(`${MS_TAG} ${TILE_TAG} mark`),
        },
        clickActions: {
            cancelFire: game$.createAction(`${MS_TAG} ${CLICK_TAG} cancel`),
            globalCancel: game$.createAction(`${MS_TAG} ${CLICK_TAG} global cancel`),
            startFire: game$.createAction(`${MS_TAG} ${CLICK_TAG} start fire`),
            fire: game$.createAction(`${MS_TAG} ${CLICK_TAG} fire`),
            alternativeFire: game$.createAction(`${MS_TAG} ${CLICK_TAG} alternative fire`),
        },
    };
    ...
    const gameInstance$ = game$.slice('instance', {
        reducers: [
            ...
            minesweeperActions.startGame.reduce((state, { safeCoordinate, mineCount, cheating }) => {
    ...
    const isGameSettingsAPreset$ = (preset) => gameSettings$.pipe(map((settings) => isTheSamePreset(preset, settings)));
    const isGameSettingsNotAPreset$ = combineLatest([gameSettings$, presets$]).pipe(map(([settings, presets]) => !Object.values(presets).some((preset) => isTheSamePreset(preset, settings))));
    const gameWidthArray$ = gameSettings$.pipe(map((settings) => [...Array.from({ length: settings.width }).keys()]));
    const gameHeightArray$ = gameSettings$.pipe(map((settings) => [...Array.from({ length: settings.height }).keys()]));
    const winHistory$ = game$.slice('history', {
        reducers: [
            minesweeperActions.addGameToHistory.reduce((state, payload) => [...state, payload]),
        ],
    });
    const highscoreEntries$ = winHistory$.pipe(withLatestFrom(presets$), map(([winHistory, presets]) => winHistory
        .map((winEntry) => {
        const presetName = Object.entries(presets).find(([, preset]) => isTheSamePreset(preset, winEntry.preset))?.[0];
        const seconds = winEntry.time / 1000;
        const minutes = Math.floor(seconds / 60);
        const remainingSeconds = seconds % 60;
        const timeStamp = `${minutes ? minutes.toString() + 'm ' : ''}${remainingSeconds}s`;
        return {
            title: presetName ?? 'Custom',
            description: `${winEntry.cheated ? 'Cheater ' : ''}size: ${winEntry.preset.height}*${winEntry.preset.width}, mines: ${winEntry.preset.mineCount}`,
            timeStamp,
            time: winEntry.time,
        };
    })
    ...
    const gameWon$ = isGameWon$.pipe(distinctUntilChanged(), filter((won) => won));
    const isGameLost$ = gameState$.pipe(map(isGameLost));
    const isGameEnded$ = gameState$.pipe(map((gameState) => isGameLost(gameState) || isGameWon(gameState)));
    const gameEnded$ = isGameLost$.pipe(distinctUntilChanged(), filter((ended) => ended));
    ...
    const getNeighbouringCoordinates = (coordinate) => Object.values(Coordinate.directions).map((direction) => ({
        x: coordinate.x + direction.x,
        y: coordinate.y + direction.y,
    }));
    const getNeighbouringCoordinateKeys = (tiles, coordinate) => getNeighbouringCoordinates(coordinate)
        .map(Coordinate.keyOf)
        .filter((neighbourKey) => Object.hasOwn(tiles, neighbourKey));
    const getNeighbouringTiles = (tiles, coordinate) => getNeighbouringCoordinateKeys(tiles, coordinate)
        .map((tileKey) => tiles[tileKey])
        .filter(isNotNullish);
    /**
     * Collects all tiles that either have no neighbouring mines or a neighbour of such tile
     */
    const spillOnSafeTiles = (tiles, key, checked = new Set()) => {
        ...
        if (!tile.isMine && tile.value === 0) {
            return [
                key,
                ...getNeighbouringCoordinateKeys(tiles, tile).flatMap((neighbour) => spillOnSafeTiles(tiles, neighbour, checked)),
            ];
        }
        else if (tile.isMine) {
            return [];
    ...
    const tilesSlice$ = gameInstance$.slice('tiles', {
        reducers: [
            minesweeperActions.tileActions.depressTile.reduce(entitySliceReducerWithPrecompute((state, payload) => ({
            ...
            minesweeperActions.tileActions.markTile.reduce(entitySliceReducerWithPrecompute((state, payload) => ({
                neighbours: getNeighbouringCoordinateKeys(state, payload),
            ...
            minesweeperActions.clickActions.cancelFire.reduce(entitySliceReducerWithPrecompute((state, revealedTileCoord) => ({
            ...
            minesweeperActions.tileActions.revealTile.reduce(entitySliceReducerWithPrecompute((state, revealedTileCoord) => {
                ...
                const flaggedNeighbours = neighbours.filter((neighbour) => isFlagTileMark(neighbour.mark)).length;
                const uncertainNeighbours = neighbours.filter((neighbour) => isQuestionTileMark(neighbour.mark)).length;
                const canRevealNeighbours = neighbouringMines === flaggedNeighbours &&
            ...
            minesweeperActions.clickActions.globalCancel.reduce(entitySliceReducer((_key, tile) => {
                return tile.pressed ? { ...tile, pressed: false } : tile;
            })),
        ],
    });
    const dicedTiles = tilesSlice$.dice(initialTile, {
        getAllKeys: (slice) => Object.keys(slice),
        getNextKey: (_keys) => '0,0',
    });
    const tiles$ = tilesSlice$.pipe(map((tiles) => Object.values(tiles)));
    const tilesFlagged$ = tiles$.pipe(map((tiles) => tiles.filter((tile) => isFlagTileMark(tile.mark)).length));
    ...
    const remainingMines$ = combineLatest([mineCount$, tilesFlagged$]).pipe(map(([mineCount, tilesFlagged]) => mineCount - tilesFlagged));
    const smileyState$ = combineLatest([gameState$, isATilePressed$]).pipe(map(([gameState, isATilePressed]) => {
        if (isATilePressed) {
     ...
     */
    game$.createEffect(merge(documentPointerUp$, documentMouseLeave$).pipe(map(() => minesweeperActions.clickActions.globalCancel.makePacket(undefined)), map(() => undefined)));
    /**
     * Start the game at a safe tile and generate mines
     */
    game$.createEffect(minesweeperActions.clickActions.fire.pipe(ifLatestFrom(gameState$, isGameReadyToStart), withLatestFrom(gameInstance$, cheating$), map(([tile, preset, cheating]) => minesweeperActions.startGame.makePacket({
     ...
     */
    game$.createEffect(minesweeperActions.clickActions.startFire.pipe(ifLatestFrom(isGameEnded$, (isGameEnded) => !isGameEnded), map((click) => minesweeperActions.tileActions.depressTile.makePacket(click))));
    /**
     * Sanitize left clicks into reveal clicks
     */
    game$.createEffect(minesweeperActions.clickActions.fire.pipe(ifLatestFrom(isGameEnded$, (isGameEnded) => !isGameEnded), map((click) => minesweeperActions.tileActions.revealTile.makePacket(click))));
    /**
     * Sanitize right clicks
     */
    game$.createEffect(minesweeperActions.clickActions.alternativeFire.pipe(ifLatestFrom(isGameEnded$, (isGameEnded) => !isGameEnded), map((click) => minesweeperActions.tileActions.markTile.makePacket(click))));
    /**
     ...
     */
    const timeElapseTickrate$ = documentPointerDown$.pipe(take(1), map(() => TIME_TICKRATE_MS), startWith(0), shareReplay(1));
    /**
     * Elapse time
     */
    game$.createEffect(gameState$.pipe(withLatestFrom(timeElapseTickrate$), switchMap(([gameState, timeElapseTickrate]) => isGameOngoing(gameState)
        ? timer(timeElapseTickrate, TIME_TICKRATE_MS).pipe(takeUntil(gameEnded$), map(() => true))
        : of(false)), map((running) => running
        ? minesweeperActions.incrementTimer.makePacket(TIME_TICKRATE_MS)
        : undefined)));
    /**
     * Add won game to the gamehistory
     */
    game$.createEffect(gameWon$.pipe(withLatestFrom(elapsedTime$, gameInstance$), map(([, time, gameInstance]) => ({
    ...
    game$.createEffect(cheating$.pipe(filter((cheating) => cheating), map(() => cheated$.setAction.makePacket(true))));
    game$.createEffect(unlockedResize$.pipe(map((unlockedResize) => {
