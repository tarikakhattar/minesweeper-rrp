<script>import { Button, ColorPicker, Select, w2kStandardColorScheme, } from '@w2k/ui';
import { map } from 'rxjs';
import { createEventDispatcher } from 'svelte';
import AppearancePreview from './appearance-preview.svelte';
import { defaultDesktopColorScheme, desktopColorSchemeSelectOptions, } from './color-scheme.interface';
export let desktopSlice;
$: allSchemeNames$ = desktopSlice.dicedSchemes.items$.pipe(map((items) => {
    const sortedItems = items.sort((a, b) => a.displayName.localeCompare(b.displayName));
    const sortedEntries = sortedItems.map((data) => [
        data.key,
        data.displayName,
    ]);
