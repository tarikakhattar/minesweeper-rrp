<script>import { Button, ButtonLook, Window, desktopColorSchemeToCssVariables, joinStyleMap, } from '@w2k/ui';
export let desktopColorScheme;
</script>
...
<style>.appearance-preview {
  ...
  box-shadow: inset -1px -1px var(--win-3d-objects-color-lighter-2), inset 1px 1px var(--win-3d-objects-color-darker-1), inset -2px -2px var(--win-3d-objects-color), inset 2px 2px var(--win-shadow-color);
}
.appearance-preview > * {
  grid-row: 1;
  grid-column: 1;
...
.example-message-box-window-content span {
...
.example-active-window-content textarea {
  resize: none;
  width: 100%;
  height: calc(100% - 3px);
  overflow-y: scroll;
