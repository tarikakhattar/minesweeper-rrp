...
const isActionPacket = (actionPacket, registeredInActionMap) => {
  return actionPacket !== void 0 && common.isNotNullish(actionPacket.type) && (registeredInActionMap?.has(actionPacket.type) ?? true);
};
...
const hasKey = (parent, key) => {
  return common.isNotNullish(key) && common.isNotNullish(parent) && typeof parent === "object" && Object.hasOwn(parent, key);
};
function ifLatestFrom(input, condition) {
  return (source) => {
    return source.pipe(
      rxjs.withLatestFrom(input),
      rxjs.filter(([sourceResult, inputResult]) => condition(inputResult, sourceResult)),
      rxjs.map(([a]) => a)
    );
  };
}
...
class Action extends rxjs.Subject {
  //	override subscribe;
  /**
  ...
  constructor(type, config = DEFAULT_ACTION_CONFIG) {
    ...
    if (common.isNotNullish(this.config.pauseWhile)) {
      this.actionPipeline = this.actionPipeline.pipe(
        ifLatestFrom(this.config.pauseWhile, (paused) => !paused)
      );
    }
    if (common.isNotNullish(this.config.throttleTime)) {
      this.actionPipeline = this.actionPipeline.pipe(
        rxjs.throttleTime(this.config.throttleTime, rxjs.asyncScheduler, {
          leading: true,
          trailing: true
        })
      );
  ...
  config;
  scope;
  registrations = new rxjs.Subscription();
  /**
   * This will emit every action of this type, both direct dispatches and
   * effect dispatches
   */
  get listenPackets$() {
    return this.scope?.listen$(this) ?? rxjs.EMPTY;
  }
  /**
   ...
   */
  getFilter() {
    return (source) => source.pipe(rxjs.filter((value) => value.type === this.type));
  }
  static makeFilter(...actions) {
    const allowedTypes = new Set(actions.map((action) => action.type));
    return (source) => source.pipe(rxjs.filter((value) => allowedTypes.has(value.type)));
  }
...
const isReduceActionSliceSnapshot = (t) => common.isNotNullish(t.actionPacket);
...
const normalizeSliceDirection = (sliceDirection) => {
  if (typeof sliceDirection === "object") {
    ...
    const selector = (state) => {
      return common.isNotNullish(state) && typeof state === "object" ? state[key] : void 0;
    };
...
class Slice extends rxjs.Observable {
  ...
  manualPause$;
  pause$;
  keyedSlices$ = new rxjs.BehaviorSubject({});
  slices$ = this.keyedSlices$.pipe(rxjs.map((keyedSlices) => Object.values(keyedSlices)));
  subscribe;
  ...
  constructor(options) {
    ...
    this.nullishParentPause$ = new rxjs.BehaviorSubject(false);
    this.manualPause$ = new rxjs.BehaviorSubject(false);
    this.pause$ = rxjs.combineLatest([this.nullishParentPause$, this.manualPause$]).pipe(
      rxjs.map(([nullishParentPause, manualPause]) => nullishParentPause || manualPause)
    );
    ...
    this.observableState$ = this.state$.pipe(rxjs.distinctUntilChanged());
    ...
    this.sliceReducingActions$ = this.reducerConfigurations$.pipe(
      rxjs.map((reducerConfigurations) => [
        ...new Set(reducerConfigurations.map((r) => r.action.type))
      ]),
      rxjs.shareReplay(1)
    );
    this.downStreamReducers$ = this.slices$.pipe(
      rxjs.withLatestFrom(this.sliceReducingActions$),
      rxjs.switchMap(([slices, sliceReducingActions]) => {
        return slices.length > 0 ? rxjs.combineLatest(slices.map((next) => next.slice.downStreamReducers$)).pipe(
          rxjs.map((subSliceReducer) => [
            ...sliceReducingActions,
            ...subSliceReducer.flat()
          ])
        ) : rxjs.of(sliceReducingActions);
      }),
      rxjs.shareReplay(1)
      // computed from a behaviorSubject and another computed field
    );
    const slicesWithDownStreamReducers$ = this.slices$.pipe(
      rxjs.switchMap((sliceRegistrations) => {
        return sliceRegistrations.length > 0 ? rxjs.combineLatest(
          sliceRegistrations.map(
            (sliceRegistration) => sliceRegistration.slice.downStreamReducers$.pipe(
              rxjs.map((downStreamReducers) => ({
                downStreamReducers,
                sliceRegistration
              }))
            )
    ...
    const dispatchAndSlices$ = schedulingDispatcher$.pipe(
      rxjs.switchMap(
        (actionPacket) => slicesWithDownStreamReducers$.pipe(
          rxjs.take(1),
          rxjs.map((slicesWithDownStreamReducers) => ({
            actionPacket,
    ...
    const zippedDispatch = dispatchAndSlices$.pipe(
      rxjs.switchMap(({ slicesWithDownStreamReducers, actionPacket }) => {
        const neededChildSlices = filterSliceRegistrationBasedOnActionTypeSupport(
          slicesWithDownStreamReducers,
          actionPacket.type
        );
        return neededChildSlices.length > 0 ? rxjs.zip(neededChildSlices).pipe(
          rxjs.map((sliceChanges) => ({ sliceChanges, actionPacket }))
        ) : rxjs.of({ sliceChanges: [], actionPacket });
      })
    );
    this.activePipeline = zippedDispatch.pipe(
      rxjs.withLatestFrom(this.state$, this.sliceReducer$),
      rxjs.map(
        ([
      ...
      rxjs.catchError((error, pipeline$) => {
...
`, error);
    ...
    this.pipeline = this.pause$.pipe(
      rxjs.switchMap((paused) => paused ? this.inactivePipeline : this.activePipeline),
      rxjs.tap((snapshot) => {
    ...
    this.plugins$ = new rxjs.BehaviorSubject(this.initialPlugins);
    this.parentListener = this.parentCoupling?.rawParentState.pipe(
      ...
      rxjs.filter(common.isNotNullish),
      rxjs.map((parentState) => this.parentCoupling?.slicer.selector(parentState)),
      rxjs.distinctUntilChanged(),
      rxjs.tap((stateFromParent) => {
    ...
    this.subscribe = this.observableState$.pipe(rxjs.filter(common.isNotNullish)).subscribe.bind(this.observableState$);
  ...
  createEffect(packet$) {
    const pausablePacket$ = this.paused$.pipe(
      rxjs.switchMap((isPaused) => isPaused ? rxjs.NEVER : packet$)
    );
  ...
  dice(initialState, diceConstructOptions) {
    ...
    const keys$ = this.pipe(
      rxjs.map((state) => diceConstructOptions.getAllKeys(state)),
      rxjs.distinctUntilChanged(fastArrayComparator)
    );
    const items$ = keys$.pipe(
      rxjs.map((keys2) => keys2.map((key) => get(key))),
      rxjs.switchMap((slices) => slices.length > 0 ? rxjs.combineLatest(slices) : rxjs.of([]))
    );
    const count$ = keys$.pipe(rxjs.map((keys2) => keys2.length));
    const some$ = (predicate) => items$.pipe(rxjs.map((items) => items.some(predicate)));
    const every$ = (predicate) => items$.pipe(rxjs.map((items) => items.every(predicate)));
    const selectOnceDefined = (key) => rxjs.firstValueFrom(
      keys$.pipe(
        rxjs.filter((keys2) => keys2.includes(key)),
        rxjs.map(
          () => this.slice(
            key,
            sliceOptions
...
class Scope {
   ...
   */
  createEffect(action) {
    const source = rxjs.scheduled(action, rxjs.asapScheduler).pipe(
      rxjs.tap((packet) => {
...
`,
  ...
  registerAction(action, registerFromAction = false) {
    ...
    const subscription = action.listen$.pipe(
      rxjs.map((payload) => action.makePacket(payload)),
      rxjs.finalize(() => this.actionMap.delete(action.type)),
      rxjs.tap((next) => {
...
exports.DEFAULT_ACTION_CONFIG = DEFAULT_ACTION_CONFIG;
exports.PremadeGetNext = PremadeGetNext;
exports.Scope = Scope;
exports.Slice = Slice;
exports.TINYSLICE_DEFAULT_PREFIX = TINYSLICE_DEFAULT_PREFIX;
exports.TINYSLICE_INTERNAL_PREFIX = TINYSLICE_INTERNAL_PREFIX;
exports.TINYSLICE_PREFIX = TINYSLICE_PREFIX;
exports.entitySliceReducer = entitySliceReducer;
exports.entitySliceReducerWithPrecompute = entitySliceReducerWithPrecompute;
exports.fastArrayComparator = fastArrayComparator;
exports.getNextKeyStrategy = getNextKeyStrategy;
exports.getNextLargestNumber = getNextLargestNumber;
exports.getNextNumberLikeStringKey = getNextNumberLikeStringKey;
exports.getNextSmallestNumber = getNextSmallestNumber;
exports.getObjectKeys = getObjectKeys;
exports.getObjectKeysAsNumbers = getObjectKeysAsNumbers;
exports.hasKey = hasKey;
exports.ifLatestFrom = ifLatestFrom;
exports.includesArrayComparator = includesArrayComparator;
exports.isActionPacket = isActionPacket;
exports.isNonNullable = isNonNullable;
exports.isNullish = isNullish;
exports.isReduceActionSliceSnapshot = isReduceActionSliceSnapshot;
exports.normalizeSliceDirection = normalizeSliceDirection;
