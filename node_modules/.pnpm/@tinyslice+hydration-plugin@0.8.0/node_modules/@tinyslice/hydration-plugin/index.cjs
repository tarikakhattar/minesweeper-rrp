...
class TinySliceHydrationPlugin {
  ...
  runMigrations() {
    ...
    for (const migration of this.options.migrations) {
      ...
      if (common.isNotNullish(stateToBeMigrated)) {
        console.group(
          `${core.TINYSLICE_PREFIX} Running migration from ${migration.fromKey} to ${migration.toKey}`
        );
  ...
  register = (hooks) => {
    ...
    if (this.options.debounceTime) {
      this.pipeline = this.pipeline.pipe(rxjs.debounceTime(this.options.debounceTime));
    }
...
exports.TinySliceHydrationPlugin = TinySliceHydrationPlugin;
