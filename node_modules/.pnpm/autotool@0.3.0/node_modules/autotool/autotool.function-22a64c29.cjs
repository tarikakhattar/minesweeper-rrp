...
const autotoolPlugin = require("autotool-plugin");
...
const defaultPlugin = require("autotool-plugin-default");
...
const consolidateElementsAndFilterOutNonExecutables = (elements, workspacePackage, executorMap, options) => {
  return [...executorMap.values()].flatMap((executor) => {
    ...
    if (executor.consolidate) {
      ...
      if (common.isNullish(consolidated)) {
        return [];
...
const evidenceMap = {
  pnpm: [
    (rootWorkspacePackage) => node_fs.existsSync(node_path.join(rootWorkspacePackage.packagePath, "pnpm-lock.yaml")),
    (rootWorkspacePackage) => rootWorkspacePackage.packageJson.packageManager?.startsWith("pnpm") ?? false
  ],
  npm: [
    (rootWorkspacePackage) => node_fs.existsSync(node_path.join(rootWorkspacePackage.packagePath, "package-lock.json")),
    (rootWorkspacePackage) => rootWorkspacePackage.packageJson.packageManager?.startsWith("npm") ?? false
  ],
  yarn: [
    (rootWorkspacePackage) => node_fs.existsSync(node_path.join(rootWorkspacePackage.packagePath, "yarn.lock")),
...
const discoverPackageManager = async (rootWorkspacePackage, options) => {
  ...
  const results = await common.asyncMap(
    Object.entries(evidenceMap),
    async ([packageManagerName, evidences]) => {
      const evidenceFound = await common.asyncMap(
        evidences,
...
const elementAndPluginFilter = (workspacePackage, filter, plugin) => {
  ...
  if (common.isNotNullish(filter.packageJsonFilter)) {
    const packageJsonMatch = match.match(
...
const isElementUntargeted = (element, executorMap) => {
  return common.isNullish(element.targetFile) && common.isNullish(element.targetFilePatterns) && common.isNullish(executorMap.get(element.executor)?.defaultTarget);
};
const isElementTargeted = (element, executorMap) => {
  return common.isNotNullish(element.targetFile) || common.isNotNullish(element.targetFilePatterns) || common.isNotNullish(executorMap.get(element.executor)?.defaultTarget);
};
...
const normalizeElementTargets = async (workspacePackageWithElements, executorMap) => {
  ...
  const elements = await common.asyncFilterMap(elementsWithTargeting, async (packageElement) => {
    const targetFiles = [];
    ...
    if (packageElement.element.targetFilePatterns) {
      const matchedFiles = await globby.globby(packageElement.element.targetFilePatterns, {
...
const assignElementsToTargets = async (workspacePackages, context, options) => {
  const workspacePackagesWithElements = workspacePackages.filter(
    (workspacePackage) => autotoolPlugin.isManagedContent(JSON.stringify(workspacePackage.packageJson))
  ).map((workspacePackage) => filterElementsForPackage(workspacePackage, context.plugins));
  return common.asyncMap(
...
const executeElementsOnPackage = async (packageElements, rootWorkspacePackage, executorMap, elementOptions, options) => {
  ...
  await Promise.allSettled(
    targetedEntries.map(async ([targetFile, elements]) => {
      const targetFilePathAbsolute = node_path.join(
        packageElements.workspacePackage.packagePath,
        targetFile
      );
      const bearsTheMark = await autotoolPlugin.isManagedFile(targetFilePathAbsolute);
      if (!bearsTheMark) {
        if (options.force) {
          options.logger.warn(
            `Target file ${targetFile} at ${packageElements.workspacePackage.packagePath} bears no mark ("${autotoolPlugin.AUTOTOOL_MARK}") but it's ignored because '--force' was used.`
          );
        } else {
          options.logger.warn(
            `Target file ${targetFile} at ${packageElements.workspacePackage.packagePath} bears no mark ("${autotoolPlugin.AUTOTOOL_MARK}"), skipping!`
          );
          return;
        }
      }
      const target = {
        ...
        targetFilePath: node_path.relative(options.cwd, targetFilePathAbsolute),
        targetFilePathAbsolute
      };
  ...
  const finalPackageJson = await fs.readJson(
  ...
  const someDependencyChanged = packageJsonDiff.some(
    (difference) => workspaceTools.isPackageJsonDependencyField(difference.path[0])
  );
  const addedAutotoolPlugins = packageJsonDiff.map((difference) => {
    const lastPathFragment = difference.path.at(-1);
    if (difference.type === "CREATE" && workspaceTools.isPackageJsonDependencyField(difference.path[0]) && typeof lastPathFragment === "string" && lastPathFragment.includes(autotoolPlugin.AUTOTOOL_PLUGIN_NAME_PREFIX)) {
...
const findInstalledPlugins = async (rawOptions) => {
  ...
  const results = await globby.globby(
    [
      `node_modules/${autotoolPlugin.AUTOTOOL_PLUGIN_NAME_PREFIX}-*`,
      `node_modules/@*/${autotoolPlugin.AUTOTOOL_PLUGIN_NAME_PREFIX}*`
...
const isAutotoolPluginObject = (plugin) => {
  ...
  return typeof assumed === "object" && !Array.isArray(assumed) && common.isObject(assumed) && Array.isArray(assumed.elements);
};
...
const loadInstalledPlugins = async (plugins, options) => {
  const modules = await common.asyncFilterMap(
    plugins,
    (name) => import(name).then((mod) => ({ mod, name }))
  );
  const loadedPlugins = await common.asyncFilterMap(modules, async ({ mod, name }) => {
    const modulePrefDefault = await (mod.default ?? mod);
    const loadedPlugins2 = await (Array.isArray(modulePrefDefault) ? common.asyncFilterMap(
...
const validate = async (workspacePackagesWithElementsByTarget, context, elementOptions, options) => {
  const unflattenedErrors = await common.asyncMap(
    context.validators,
    async (validator) => {
      const elementErrors = await common.asyncMap(
...
const autotool = async (rawOptions) => {
  const options = autotoolPlugin.normalizeAutotoolOptions(rawOptions);
  if (options.maxAllowedRecursion <= 0) {
    options.logger.info("recursive limit reached!");
    return;
  }
  const workspacePackages = await workspaceTools.collectWorkspacePackages(rawOptions);
  ...
  if (isValid) {
    ...
    if (results.some((result) => result.someDependencyChanged)) {
      ...
      node_child_process.execSync(packageManager.installCommand, {
        stdio: "inherit"
      });
...
exports.autotool = autotool;
exports.autotoolPluginFilterPredicate = autotoolPluginFilterPredicate;
exports.checkIfTheresAnElementWithoutValidExecutor = checkIfTheresAnElementWithoutValidExecutor;
exports.consolidateElementsAndFilterOutNonExecutables = consolidateElementsAndFilterOutNonExecutables;
exports.createIsPackageElementTargeted = createIsPackageElementTargeted;
exports.discoverPackageManager = discoverPackageManager;
exports.elementAndPluginFilter = elementAndPluginFilter;
exports.executeElementsOnPackage = executeElementsOnPackage;
exports.filterElementsForPackage = filterElementsForPackage;
exports.findInstalledPlugins = findInstalledPlugins;
exports.groupAndConsolidateElementsByTargetFile = groupAndConsolidateElementsByTargetFile;
exports.installCommands = installCommands;
exports.isAutotoolPluginObject = isAutotoolPluginObject;
exports.isElementTargeted = isElementTargeted;
exports.isElementUntargeted = isElementUntargeted;
exports.isPackageElementTargeted = isPackageElementTargeted;
exports.isPackageElementUntargeted = isPackageElementUntargeted;
exports.isRootWorkspacePackage = isRootWorkspacePackage;
exports.loadInstalledPlugins = loadInstalledPlugins;
exports.loadPlugin = loadPlugin;
exports.mapRecord = mapRecord;
exports.normalizeElementTargets = normalizeElementTargets;
