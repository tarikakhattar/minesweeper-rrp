...
const autotool_function = require("../autotool.function-22a64c29.cjs");
require("@alexaegis/fs");
require("@alexaegis/workspace-tools");
require("microdiff");
require("node:child_process");
require("autotool-plugin-default");
const name = "autotool";
...
const yargsForAutotool = (yargs) => {
  return cliTools.yargsForCollectWorkspacePackagesOptions(
    cliTools.yargsForLogLevelOption(cliTools.yargsForDryOption(cliTools.yargsForForceOption(cliTools.yargsForCwdOption(yargs))))
  ).option("dryish", {
    boolean: true,
    default: false,
...
const yarguments = cliTools.YargsBuilder.withDefaults(packageJson).add(yargsForAutotool).build();
void (async () => {
  ...
  if (options.listPlugins) {
    const installedPlugins = await autotool_function.findInstalledPlugins(options);
    console.info(installedPlugins.join("\n"));
  } else {
    await autotool_function.autotool(options);
  }
})();
