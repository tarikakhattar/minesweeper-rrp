...
const yarguments = cliTools.YargsBuilder.withDefaults(packageJson).add(cliTools.yargsForCollectWorkspacePackagesOptions).add(
  (y) => y.option("skipNodeModules", {
    ...
    default: index.DEFAULT_NUKE_LIST
  }).option("nukeMore", {
    array: true,
    type: "string",
    ...
    default: index.DEFAULT_NUKE_GLOBS
...
).add(cliTools.yargsForLogLevelOption);
void (async () => {
  const options = await yarguments.build().parseAsync();
  const logger = logging.createLogger({ name: "nuke ☢", minLevel: options.logLevel });
  await index.nuke({ ...options, logger });
})();
