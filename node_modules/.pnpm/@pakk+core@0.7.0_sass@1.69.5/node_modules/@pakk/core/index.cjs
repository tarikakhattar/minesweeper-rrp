...
const stripFileExtension = (name, options) => {
  const extension = name.endsWith(dtsExtension) && options?.stripDts !== false ? dtsExtension : node_path.extname(name);
...
const createExportMapFromPaths = (pathsFromBase, options) => {
  ...
  for (const path of pathsFromBase) {
    const key = options.keyKind === "extensionless-filename-only" ? stripFileExtension(node_path.basename(path)) : "./" + stripFileExtension(path);
    const pathVariants = {
      "development-to-source": "./" + node_path.posix.join(options.srcDir, basePath, path),
      // The original full path, not used by default but there's an option if preferred
      "development-to-dist": "./" + node_path.posix.join(options.outDir, path),
      // It is assumed that files in the outDir replicate their folder structure from the srcDir
      "distribution-to-dist": "./" + path
    };
    if (options.shimDir) {
      pathVariants["development-to-shim"] = "./" + node_path.join(options.shimDir, path);
    }
...
const enterPathPosix = (path, enterCount = 1) => {
  const explodedPath = node_path.posix.normalize(path).split(node_path.posix.sep);
  const directoryCount = explodedPath.length - 1;
  explodedPath.splice(0, Math.min(enterCount, directoryCount));
  const prefix = path.startsWith("./") ? "./" : "";
  return prefix + node_path.posix.join(...explodedPath);
};
...
class AutoCopyLicense {
  ...
  examinePackage(workspacePackage) {
    ...
    const possibleLicenseFileLocations = pathsOfInterest.flatMap(
      (path) => possibleLiceseFileNames.map((fileName) => node_path.join(path, fileName))
    );
  ...
  async process(_packageJson, pathContext) {
    if (pathContext.packageJsonKind === PACKAGE_JSON_KIND.DISTRIBUTION) {
      ...
      const licenseFileDestination = node_path.join(
        fs.toAbsolute(this.context.outDir, this.context),
        node_path.basename(this.licensePath)
      );
      try {
...
class AutoExport {
  ...
  async examinePackage(_packageJson) {
    const absoluteExportBaseDir = fs.toAbsolute(
      node_path.posix.join(this.context.srcDir, this.options.exportBaseDir),
      {
    ...
    return {
      bundlerEntryFiles: entryFiles.reduce((acc, entryFile) => {
        const path = node_path.posix.join(this.context.srcDir, this.options.exportBaseDir, entryFile);
  ...
  process(_packageJson, pathContext) {
    ...
    for (const [key, pathVariants] of Object.entries(this.exportMap)) {
      ...
      if (this.context.formats.includes("cjs")) {
        exportConditions.require = "./" + node_path.posix.join(dir, this.context.fileName("cjs", extensionlessFileName));
      } else {
        if (this.context.formats.includes("umd")) {
          exportConditions.require = "./" + node_path.posix.join(dir, this.context.fileName("umd", extensionlessFileName));
        } else if (this.context.formats.includes("iife")) {
          exportConditions.require = "./" + node_path.posix.join(dir, this.context.fileName("iife", extensionlessFileName));
        }
      }
      if (this.context.formats.includes("es")) {
        exportConditions.import = "./" + node_path.posix.join(dir, this.context.fileName("es", extensionlessFileName));
      }
      if (this.context.formats.includes(this.context.primaryFormat)) {
        exportConditions.default = "./" + node_path.posix.join(
          dir,
      ...
      if (this.options.svelte) {
        exportConditions["svelte"] = "./" + node_path.posix.join(
          dir,
...
const findCurrentAndRootWorkspacePackage = async (rawOptions) => {
  ...
  const packageDirName = options.cwd.slice(Math.max(0, options.cwd.lastIndexOf(node_path.sep)));
  ...
  const workspacePackage = workspace.find(
    (workspacePackage2) => workspacePackage2.packageKind === "regular" && workspacePackage2.packagePath.includes(options.cwd) && (workspacePackage2.packagePath + node_path.sep).includes(packageDirName + node_path.sep)
  );
...
class Pakk {
  ...
  async createUpdatedPackageJson(packageJsonKind) {
    ...
    const path = packageJsonKind === PACKAGE_JSON_KIND.DISTRIBUTION ? fs.toAbsolute(
      ...
      node_path.join(this.context.workspacePackage.packagePath, "package.json"),
      this.options
    );
...
exports.pakkFeatureMap = pakkFeatureMap;
